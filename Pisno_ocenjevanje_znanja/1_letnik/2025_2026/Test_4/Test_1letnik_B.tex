...
\begin{center}
    ...
    Četrto pisno ocenjevanje znanja -- ponovno \\ 1. letnik -- test B \\ 22. april 2026 \\ (Realna števila)}}}
\end{center}
...
\begin{questions}
    ...
    $$ \sqrt{6\cdot 14\cdot 21}-\left(\sqrt{3}-1\right)^2+\dfrac{\sqrt{3}}{2+\sqrt{3}}-\left(3+\sqrt{3}\right)\left(1-2\sqrt{3}\right) $$
    ...
    \question[4]
    Rešite enačbo.
    $$ \dfrac{x-4}{x+1}=\dfrac{2x^2}{x^2-x-2} - \dfrac{x+4}{x-2} $$
    ...
    \question[5]
    Izračunajte natančno vrednost izraza.
    $$ \left\lvert \lvert\sqrt{5}-2\rvert+\sqrt{5}\right\rvert - \left\lvert \lvert\sqrt{3}-4\rvert+\sqrt{3}\right\rvert $$
    ...
    % 9. vprašanje
    \question[5]
    Prvi delavec sam pozida steno v $10$ urah, drugi v $12$ urah, tretji v $5$ urah. Delavci
    skupaj začnejo zidati steno. Po eni uri tretji delavec odide, pridruži pa se četrti delavec.
    Skupaj s prvim in drugim delavcem nato končajo steno v dveh urah. V kolikšnem času četrti delavec
    pozida steno?
    ...
    % dodatno vprašanje
    \bonusquestion[3]
    \textit{Dodatna naloga:} Dani sta števili $a=-8$ in $b=25$.
        Izračunajte vrednost izraza $$\left||a|-2\sqrt{b}\right|-\frac{1}{2}\left|2-\sqrt{2|a|}\right|.$$
