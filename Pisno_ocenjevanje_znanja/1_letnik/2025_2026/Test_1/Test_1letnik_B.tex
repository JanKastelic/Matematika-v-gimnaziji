...
\begin{center}
    ...
    Prvo pisno ocenjevanje znanja \\ 1. letnik -- test B \\ 16. oktober 2025 \\ (Osnove logike in teorije množic, Naravna in cela števila)}}}
\end{center}
...
\begin{questions}
    ...
    \question[4]
    Določite pravilnost izjave $( A \land \lnot B)\Rightarrow (\lnot A \Leftrightarrow B)$ glede na izbor izjav $A$ in $B$.
    ...
    \begin{parts}
        ...
        \part 
        Število $1$ je praštevilo ali število $5$ ni večkratnik števila $1$.\\
        \part
    ...
    $20$ dijakov ima \textit{Atlas sveta}, $7$ jih ima \textit{Atlas Jugoslavije} in $18$ \textit{Atlas Slovenije}. Kar $12$ jih ima \textit{Atlas sveta} in \textit{Atlas Slovenije}, $4$ imajo \textit{Atlas Jugoslavije} in \textit{Atlas sveta},
    ...
    % 5. vprašanje
    \question
    Naj bo $\mathcal{U}=\mathbb{N}_{22}$. Dane so množice $\mathcal{A}=\left\{x;\ x=2n \land 4\leq n \leq10\right\}$, $\mathcal{B}=\left\{x;\ x=4k-3 \land 3\leq k< 6\right\}$ in $\mathcal{C}=\left\{x;\ x=5m \land 1<m\leq 4\right\}$. 
    \begin{parts}
        \part[3]
        Zapišite dane množice z naštevanjem elementov.
        \part[4]
        Določite naslednje množice: $\mathcal{C}\cap\mathcal{A}$, $\mathcal{C}\setminus\mathcal{A}$, $(\mathcal{C}\cup\mathcal{A})\setminus\mathcal{B}$ in $(\mathcal{A}\cup\mathcal{B})^\complement$.
        \part[1]
    ...
    V tednu, ko naj bi bil tečaj smučanja izveden, je zbolelo $11$ parov, ki so bili vključeni v tečaj, ostali so se tečaja udeležili.
