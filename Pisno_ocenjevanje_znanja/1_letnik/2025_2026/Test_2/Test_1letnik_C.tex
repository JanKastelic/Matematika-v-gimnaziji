...
\begin{center}
    ...
    Drugo pisno ocenjevanje znanja \\ 1. letnik -- test C \\ 4. december 2025 \\ (Potence in izrazi; Deljivost)}}}
\end{center}
...
\begin{parts}
    \part[4]
    $\left(-1\right)^{2025}\cdot\left(\left(-2\right)^3+\left(-5\right)^2\right)-1^{2026}-\left(\left(-3\right)^2\cdot\left(-2\right)-\left(-4\right)^2\right)$
    ...
    \part[3]
    $7\cdot 2^{2n+1}-2\cdot 4^{n+1}-2\cdot 2^{2n+2}$
    ...
    % 2. vprašanje
    \question[5]
    Skrčite izraz in rezultat razstavite: $(7a-2b)^2+(2b-5a)(2b+5a)-6(4a^2-b^2)$.
    ...
    % 4. vprašanje
    \question
    Določite neznani števki, da bo število $\overline{a7777777777b}$ deljivo
    \begin{parts}
        \part[1]
        s številom $2$.
...
% 5. vprašanje
    ...
    \question[4]
    Naj bo $a=2^2\cdot 3^2\cdot 175$, $b=3^4\cdot 5^4\cdot 7\cdot 13$ in $c=36\cdot 625$. 
    ...
    % 6. vprašanje
    \question[3]
    Število $m$ da ostanek $2$ pri deljenju s $3$, število $n$ pa da pri deljenju z $9$ ostanek $6$.
    Pokažite, da je število $n+3m$ deljivo s $3$.
    ...
    \textit{Dodatna naloga:} Izračunajte in rezultat zapišite v ustreznem številskem sestavu.
                $$ 3724_{(8)}+ 254_{(6)}= \quad \quad \quad \quad _{(2)} $$
